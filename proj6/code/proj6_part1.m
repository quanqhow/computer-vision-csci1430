...
im = imdb.images.data(:,:,:,batch); % im is a 64x64x1x50 array
labels = imdb.images.labels(1,batch) ;

num_to_flip = randi([10 30]); % flip a random number of images
inds = randperm(50,num_to_flip); % range of 1,50, size is 1xnum_to_flip 
for i = 1:length(inds)
    ind = inds(i);
    flip = im(:,:,:,ind);
    flip = fliplr(flip); %fliplr, B = fliplr(A);
    im(:,:,:,ind) = flip;
end
