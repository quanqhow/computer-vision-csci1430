...
% w should be a 1116x1 weighting vector
% b should be a scalar
% b = rand(1); %placeholder, delete

lambda = 0.00025;
[num_pos, feat_size1] = size(features_pos);
[num_neg, feat_size2] = size(features_neg);
labels = ones(num_pos+num_neg,1);
labels(num_pos:num_pos+num_neg,1) = -1; % labels is a column vector

training_feats = cat(1,features_pos,features_neg); % nx1116 matrix
% size(labels) % size
% size(training_feats)
[w b] = vl_svmtrain(training_feats',labels,lambda);

% W'*X(:,i)+B = same sign
% X(;,i) is a 1116x1 vector
% W' must be a 1x1116 vector, so W must be a 1116x1 vector
